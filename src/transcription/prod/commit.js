...
(async () => {
    const targetContainerName = 'onsets-and-frames'
    await updatePipeline('midi.json', targetContainerName);
    await exec(`docker build -t ${docker_name} -f Dockerfile .`)
    await exec(`docker tag ${docker_name} hitheory/${targetContainerName}:${version}`)
    await exec(`docker push hitheory/${targetContainerName}:${version}`)

    await exec(`pachctl update pipeline -f ./midi.json`);
    await exec(`pachctl put file audio-unprocessed@master:aphextwin.mp3 -f ../../audio/samples/aphextwin.mp3`);
    await wait(2);
    // await exec(`pachctl logs -p midi -f`);
})();
